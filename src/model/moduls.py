...
class FFN(nn.Module):
    def __init__(self, kernel_size, stride, dilation, padding, input_channel, dropout):
        super().__init__()

        self.channel_expand = nn.Sequential(
            nn.Conv1d(
                in_channels=input_channel, out_channels=2 * input_channel, kernel_size=1, bias=False
            ),
            GLN(2 * input_channel),
        )
        ...
        self.channel_narrow = nn.Sequential(
            nn.Conv1d(
                in_channels=2 * input_channel, out_channels=input_channel, kernel_size=1, bias=False
            ),
            GLN(input_channel),
        )
...
class MHSA(nn.Module):
    def __init__(self, embed_dim, nhead, dropout=0.0):
        ...
        self.qkv_proj = nn.Linear(in_features=embed_dim, out_features=3 * heads_dim * nhead)
        self.nhead = nhead
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self.gln(
            x + self.cross_head_linear(attn.view(B, T, -1)).transpose(1, 2)
        )  # (B, nhead * heads_dim, T)
...
class TransformerLayer(nn.Module):
    def __init__(
        ...
        if not ffn_only:
            self.mhsa = MHSA(embed_dim=input_channel, nhead=mhsa_nhead, dropout=mhsa_dropout)
        else:
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.avg_pools = nn.ModuleList(
            [
                nn.AvgPool1d(kernel_size=downsample_rate**i)
                for i in range(downsample_num_layers, 0, -1)
            ]
        )
