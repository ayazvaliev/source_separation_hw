...
from src.metrics import PESQ, STOI, SDRi, SISNRi
...
def main():
    ...
    parser.add_argument(
        "--predictions_dir",
        required=True,
        help="Path to dir with saved predictions from inference.py",
    )
    parser.add_argument("--gt_dir", required=True, help="Path to dir with GT speaker waveforms")
    parser.add_argument(
        "--mix_dir", required=True, help="Path to dir with mixes which were separated"
    )
    ...
    for format in formats:
        for s1_est_path in s1_est_dir.glob("*." + format):
            ...
            logits = torch.stack([load_audio(path) for path in [s1_est_path, s2_est_path]], dim=0)
            audio_concat = torch.stack(
                [load_audio(path) for path in [s1_gt_path, s2_gt_path]], dim=0
            )
            audio_mix = load_audio(mix_path, squeeze_channel=False)
            data_dicts.append(
                {"logits": logits, "audio_concat": audio_concat, "audio_mix": audio_mix}
            )
